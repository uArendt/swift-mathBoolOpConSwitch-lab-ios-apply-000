    ...
    override func spec() {
        ...
        describe("averageIsAbove75(_:_:)") {
            it("Takes three numbers and, if their average is over 75, returns true, otherwise returns false") {
                ...
                expect( testVC.averageIsAbove75( 76.0, two: 75.0, three: 71.9 )).to(equal(false))    // NOTE we might want to get specific about the precision & rounding they should use (eg. is 75.4 > 75?  75.4 is *definitely* greater than 75.0)
                
                expect( testVC.averageIsAbove75( 75.0, two: 75.0, three: 75.0 )).to(equal(false))    // must be *OVER* 75
                
                expect( testVC.averageIsAbove75( 75.1, two: 75.0, three: 75.0 )).to(equal(true))   // they should be using Doubles, so this is *just* over 75
                
                expect( testVC.averageIsAbove75( 80.0, two: 80.0, three: 80.0 )).to(equal(true))
            }
        }
        ...
        describe("numberGenerator(_:_:)") {
            ...
            it("Checks the validity of each paramters and returns their product as a double or, if there's a validation error, it returns 0") {
                ...
                expect(testVC.numberGenerator("3.5", b: 12)).to(equal(0.0))   // first arg is not a whole number
